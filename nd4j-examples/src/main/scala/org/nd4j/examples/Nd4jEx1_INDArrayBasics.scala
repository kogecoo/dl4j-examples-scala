...
object Nd4jEx1_INDArrayBasics {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Num. Dimensions:    " + myArray.rank) //2 dimensions -> rank 2
    println("Shape:              " + Arrays.toString(myArray.shape)) //[3,5] -> 3 rows, 5 columns
    println("Length:             " + myArray.length) // 3 rows * 5 columns = 15 total elements
    ...
    println()
    println("size(0) == nRows:   " + myArray.size(0)) //Also equivalent to: .shape()[0]
    println("size(1) == nCols:   " + myArray.size(1)) //Also equivalent to: .shape()[1]
    ...
    val val0 = myArray.getDouble(0, 1) //Get the value at row 0, column 1 - expect value 2.0 as we set this earlier
    println("\nValue at (0,1):     " + val0)

    //Finally, there are many things we can do to the array... for example adding scalars:
    val myArray2 = myArray.add(1.0) //Add 1.0 to each entry
    println("\nNew INDArray, after adding 1.0 to each entry:")
    println(myArray2)

    val myArray3 = myArray2.mul(2.0) //Multiply each entry by 2.0
