...
/**
  ...
  */
object PreprocessNormalizerExample {

  private val log = LoggerFactory.getLogger(PreprocessNormalizerExample.getClass)
  ...
  def main(args: Array[String]) {
    ...
    val recordReaderA = new CSVRecordReader(numLinesToSkip, delimiter)
    val recordReaderB = new CSVRecordReader(numLinesToSkip, delimiter)
    ...
    val labelIndex = 4
    val numClasses = 3
    val iteratorA = new RecordReaderDataSetIterator(recordReaderA, 10, labelIndex, numClasses)
    val iteratorB = new RecordReaderDataSetIterator(recordReaderB, 10, labelIndex, numClasses)
    val fulliterator = new RecordReaderDataSetIterator(recordReader, 150, labelIndex, numClasses)
    val datasetX = fulliterator.next
    val datasetY = datasetX.copy
    ...
    if (iteratorA.hasNext) {
      ...
      var firstBatch: DataSet = iteratorB.next
      log.info("\n{}", firstBatch)
