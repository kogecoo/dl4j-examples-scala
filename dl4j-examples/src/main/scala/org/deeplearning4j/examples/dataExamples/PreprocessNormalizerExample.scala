 ...
 */
class PreprocessNormalizerExample {

    private lazy val log = LoggerFactory.getLogger(classOf[PreprocessNormalizerExample])
    ...
    def main(args: Array[String]) {
        ...
        val recordReaderA: RecordReader = new CSVRecordReader(numLinesToSkip,delimiter)
        val recordReaderB: RecordReader = new CSVRecordReader(numLinesToSkip,delimiter)
        ...
        val labelIndex = 4
        val numClasses = 3
        val iteratorA: DataSetIterator = new RecordReaderDataSetIterator(recordReaderA,10,labelIndex,numClasses)
        val iteratorB: DataSetIterator = new RecordReaderDataSetIterator(recordReaderB,10,labelIndex,numClasses)
        val fulliterator: DataSetIterator = new RecordReaderDataSetIterator(recordReader,150,labelIndex,numClasses)
        val datasetX: DataSet = fulliterator.next()
        val datasetY: DataSet = datasetX.copy()
        ...
        val preProcessorIter = new NormalizerMinMaxScaler()
